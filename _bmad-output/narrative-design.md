...
| **E02** | Notebook entry corrupted | `OnEntryCorrupted(entryId)` — first corruption event fires after Night 2 if entity entered HUNTING state | Day 3+, Recovery Phase, Office (notebook) | Notebook + Entity | **Guaranteed by Day 3.** If /idn't hunt on Night 2 (very unlikely at threshold 0.7), corruption occurs on first night where entity hunts. If player somehow avoids all hunting through Night 4, force-corrupt 1 entry at Night 4 Recovery as a failsafe. | First corruption is flagged specially in UI. Subsequent corruptions are normal system behavior. |
